...
import { loginToOffice, navigateToExcelOnline } from './helpers/auth';
...
test.describe('Excel Online TODAY() Function Tests', () => {
  test('should return current date when using TODAY() function', async ({ page }) => {
    ...
    await navigateToExcelOnline(page);
    
    // Create new blank workbook if needed
    await page.click('button:has-text("New blank workbook")', {
      timeout: 10000
    });
